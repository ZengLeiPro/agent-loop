...
**implementation 必需 vs 越权 — 判断标准**：

- **不实现就跑不通 acceptance** = implementation 必需,正常做(例如 story 是"列表分页",连接池配置 / 错误兜底 / loading 状态都属于实现细节,不需要 spec 写)。
- **实现可以跑通 acceptance,但功能上扩展了 acceptance 之外** = 越权,不要做(例如 story 没说"导出 CSV",就别加导出按钮)。

边界不清时倾向不做,在 progress Notes 标"considered: X, deferred as out of story scope"留给 Judge / human 判断。
...
硬规则：**validation 未通过，task 就没有完成。** 如果**本轮内**同一 implement→validate 循环重试 3 次后仍失败(不是跨轮累计 3 次,是本轮单独计数),在 `prd.json` 中把 story 标为 BLOCKED（添加 `blocked: "<one-line reason>"` 字段），在 AGENTS.md 记录 failure mode，commit 能编译的内容并退出。下一轮会决定如何解阻或跳过。
...
`Commit:` 字段有一个 UX quirks：step 8 前还不知道 hash。两个选项,**推荐第二个**：

- step 7b 先写 `Commit: <pending>`，step 8 后用 `git commit --amend -m` 修正 hash。看起来干净,但 amend 会改 hash,如果你已经在 progress 文件其他位置引用过本轮 hash,要二次同步,容易漏。
- **(推荐)** 写 `Commit: HEAD` 作为自引用,Judge 从 `git log` 计算实际 hash。代价是 Judge 多走一步,收益是 Worker 这一轮完全不需要 amend、不会产生 hash 漂移。默认走这条。
